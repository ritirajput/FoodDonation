...
struct DonationHistoryView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                .onAppear {
                    fetchDonations(for: selectedTab)
                }
            }
    ...
    private func fetchDonations(for status: String) {
        guard let userId = Auth.auth().currentUser?.uid else { return }
        databaseRef.child("donations").queryOrdered(byChild: "status").queryEqual(toValue: status).observeSingleEvent(of: .value) { snapshot in
            guard let data = snapshot.value as? [String: Any] else {
                self.donations = []
                return
            }
            self.donations = data.compactMap { key, value in
                if let donationData = value as? [String: Any], donationData["userId"] as? String == userId {
...
struct Donation: Identifiable {
    ...
    init(id: String, data: [String: Any]) {
        ...
        self.statusColor = Donation.getStatusColor(for: self.status)
    }
